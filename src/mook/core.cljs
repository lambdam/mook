  ...
  ([store-key handler debug-info]
   (let [state-store* (as-> (use-mook-state-stores) <>
         ...
         [value set-value!] (mr/use-state #(let [value (handler @state-store*)]
                                             (log/dev-print! {:type :first-hook-call
                                                              :f 'use-state-store
                                                              :store-key store-key
                                                              :handler handler
                                                              :value value
                                 ...
                                 (fn listen-to-store [{::keys [new-state] :as data}]
                                   ...
                                   (let [handler' (.-current last-handler-ref)
                                         new-value (let [value (handler' new-state)]
                                                     (log/dev-print! {:type :watcher-call
                                                                      :f 'use-state-store
                                                                      :store-key store-key
                                                                      :handler handler
                                                                      :value value
                                     ...
                                     (when (not= new-value (.-current last-value-ref))
                                       (log/dev-print! {:type :rerender-on-watcher-call
                                                        :f 'use-state-store
                                                        :store-key store-key
                                                        :handler handler
     ...
     (when (-> first-call?-ref .-current false?)
       (let [new-value (handler @state-store*)]
         (log/dev-print! {:type :check-new-value
                          :f 'use-state-store
                          :handler handler
                          :value new-value
                          :debug-info debug-info}
         ...
         (when (not= new-value (.-current last-value-ref))
           (log/dev-print! {:type :rerender-on-new-value
                            :f 'use-state-store
                            :handler handler
                            :old-value (.-current last-value-ref)
                            :new-value new-value
